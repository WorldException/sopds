...
if [ ! -f /sopds/conf/sopds.conf ]; then

cat > /sopds/conf/sopds.conf <<EOF
...
formats    = ${formats:-'.pdf .djvu .fb2 .epub'}
...
scan_day_of_week = ${scan_day_of_week:-0}
...
# Префиксы после строки адреса, которые будут использованы для определения формата вывода (OPDS или WEB)
# по умолчанию, если никакой префикс не подходит, вывод будет в OPDS формате
opdsprefix = '/opds'
webprefix  = '/web'
...
EOF
    ...
    # подождать инициализации базы данных
    sleep 5
fi

case "$1" in
server)
    python sopdsd.py start
    tail -f ../logs/sopds-httpd.log ../logs/sopds-scand.log
    ;;
*)
    "$@"
    ;;
